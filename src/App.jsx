...
function App() {
  ...
  useEffect(() => {
    ...
    console.log(`Presentation Loop: Slide ${activeSlideId}, Duration: ${duration}ms`);
    ...
    return () => clearTimeout(timer);
  }, [isPresenting, isLooping, activeSlideId, slides]);

  const handlePaintApply = (newSrc) => {
    if (paintModalData) {
      updateElement(paintModalData.elementId, { src: newSrc });
      setPaintModalData(null);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%' }}>
      ...
      {paintModalData && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 9000 }}>
          <PaintOverModal 
            src={paintModalData.src} 
            onApply={handlePaintApply} 
            onClose={() => setPaintModalData(null)} 
          />
        </div>
      )}
    </div>
...
// ── COMPONENTE: Modal de Pincel para cobrir texto (Moved to App level) ──
